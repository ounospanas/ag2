...
class TestHelperFunctions:
    def test_update_conditional_functions(self, agent1: MagicMock) -> None:
        ...
        _update_conditional_functions(agent=agent1, messages=[])
    ...
    def test_link_agents_to_group_manager(
        ...
        _link_agents_to_group_manager(typed_agents, mock_group_chat_manager)
    ...
    def test_process_initial_messages(self, agent1: MagicMock, agent2: MagicMock, user_proxy: MagicMock) -> None:
        ...
        msgs, last_agent, names, temps = _process_initial_messages("Hello", None, typed_agents, wrapped_agents)
        ...
        msgs, last_agent, names, temps = _process_initial_messages("Hello", user_proxy, typed_agents, wrapped_agents)
        assert msgs == [{"role": "user", "content": "Hello"}]
        ...
        msgs, last_agent, names, temps = _process_initial_messages(
        ...
        with pytest.raises(ValueError, match="Invalid group agent name in last message: unknown"):
            _process_initial_messages(initial_msgs, user_proxy, typed_agents, wrapped_agents)
        ...
        msgs, last_agent, names, temps = _process_initial_messages(initial_msgs, None, typed_agents, wrapped_agents)
        assert msgs == initial_msgs
        assert last_agent is not None
    ...
    def test_setup_context_variables(
        ...
        agents = [agent1, agent2]
        typed_agents = cast(list[ConversableAgent], agents)
        _setup_context_variables(mock_tool_executor, typed_agents, mock_group_chat_manager, context_vars)
    ...
    def test_prepare_groupchat_auto_speaker(
    ...
    def test_determine_next_agent(
        self,
        mock_prep_auto_speaker: MagicMock,
        mock_get_last_speaker: MagicMock,
        ...
        mock_group_after_work = MagicMock(spec=AfterWork)
        ...
        awot_next_target = AfterWorkOptionTarget(after_work_option="terminate")
        mock_tool_executor.has_next_target.return_value = True
...
class TestInitiateGroupChat:
    ...
    def test_initiate_group_chat_flow(
        ...
        mock_groupchat_cls: MagicMock,
        mock_create_transition: MagicMock,
        mock_process_initial: MagicMock,
        ...
        mock_prepare_agents.assert_called_once_with(
            typed_agents, context_vars, True
...
@pytest.mark.asyncio
async def test_a_initiate_group_chat(agent1: MagicMock, agent2: MagicMock) -> None:

...
<%def name="deflist(s)">${indent(s)}</%def>
...
<%
        ...
        # Format parameters with line breaks and indentation
        params = func.params(annotate=show_type_annotations)
        if len(params) > 2:  # If there are multiple parameters, format them vertically
            formatted_params = ',\n    '.join(params)
            signature = f"{func.name}(\n    {formatted_params}\n){returns}"
        else:  # For few parameters, keep them on one line
